...
class visual_sudoku():
    ...
    def option_select(self):
        ...
        def validate_puzzle_num(self, puzzle_num, difficulty):
            if puzzle_num == 'random':
                url = 'http://www.menneske.no/sudoku/eng/random.html?type=' + str(difficulty)
    ...
    def new_window(self, url):
        ...
        self.display_sudoku(self.sudoku)
        
        self.window.mainloop()
        pass
    
    
    def display_sudoku(self, array):
        for row, row_values in enumerate(array, 1):
            for col, cell in enumerate(row_values, 1):
                if isinstance(cell, int):
                    if cell != 0:
                        tk.Label(self.canvas, text=cell, font=('Helvetica, 15')).place(x=row*50, y=col*50, anchor=tk.CENTER)
                        pass
                    pass
                else:
                    for num in cell:
                        tk.Label(self.canvas, text=num, font=('Helvetica, 7')).place(x=(row-1)*50+35+(num-1)%3*15, y=(col-1)*50+37+(num-1)//3*14, anchor=tk.CENTER)
                        pass
                    pass
    ...
    def solve(self):
        ...
        self.display_sudoku(sudoku_replica) #the cells (only num not list) are displayed twice... yikes..
